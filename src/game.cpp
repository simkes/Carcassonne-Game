...
void Game::init_interaction() {
    mInteraction.emplace(
        State::DEFAULT,
        std::make_unique<defaultInteraction>(
            mBoard, mGameRenderPtr->get_boardView(), mGameRenderPtr->window()));
    mInteraction.emplace(State::CARDPLACEMENT,
                         std::make_unique<cardPlacementInteraction>(
                             mBoard, mGameRenderPtr->get_boardView(),
                             &currentCardPtr, mGameRenderPtr->window()));
    mInteraction.emplace(
        State::UNITPLACEMENT,
        std::make_unique<unitPlacementInteraction>(
            mBoard, mGameRenderPtr->get_boardView(), &currentCardPtr,
            currentPlayerPtr, mGameRenderPtr->window()));
}
...
void Game::set_currentCard() {
    ...
    placedCards.back().setSprite(*getTextures().get_texture(
        textureId < 16 ? textures::ID::CARDS1 : textures::ID::CARDS2));
...
Game::Game(std::vector<std::pair<sf::String, game_model::Color>> players,
           GameRender *gameRenderPtr)
    : mGameRenderPtr(gameRenderPtr) {
...
void Game::process_events(sf::Event &event) {
    while (mGameRenderPtr->window().pollEvent(event)) {
        ...
        //                        sf::Mouse::getPosition(mGameRenderPtr->window());
        //                        sf::Vector2i cardPos =
        //                            mBoard.getEmptyPosition(position);
        //                        sf::Vector2f cardSpritePos =
        //                        transform_coordinates({(cardPos.x/(CARD_DIMENSION-1))*textures::CARD_TEXTURE_SIZE
        //                        + 74,
        //                        (cardPos.y/(CARD_DIMENSION-1))*textures::CARD_TEXTURE_SIZE
        //                        + 74});
...
void Game::change_state() {
    switch (currentState) {
        case State::DEFAULT: {
            ...
            currentPlayerPtr = &mPlayers[currentPlayerIndex];
            currentState = State::CARDPLACEMENT;
            dynamic_cast<unitPlacementInteraction *>(
                mInteraction[State::UNITPLACEMENT].get())
                ->setCurrentPlayer(currentPlayerPtr);
            endOfState = false;
            break;
        }
